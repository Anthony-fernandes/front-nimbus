...
export const Route = createFileRoute("/permission-blocks")({
  head: () => ({ meta: [{ title: "Blocos de permissões · NimbusDesk" }] }),
...
function PermissionBlocksPage() {
  ...
  function openEdit(block: PermissionBlock) {
  ...
  return (
    <AppShell>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          setDialogOpen(open);
          if (!open) { setSelectedId(null); setFormData(emptyForm); }
        }}
      >
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {selectedBlock ? `Editar ${selectedBlock.name}` : "Novo bloco de permissões"}
            </DialogTitle>
          </DialogHeader>
            ...
            {canManage ? (
              <div className="flex gap-2 pt-2">
                ...
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => { setDialogOpen(false); setFormData(emptyForm); }}
                >
                  Cancelar
                </Button>
              </div>
